...
(function (global, factory) {
    ...
    ***************************************************************************** */
     ...
     */
    class Environment {
        constructor(importObject = {}, logger = () => {}) {
            /**
...
`;
    ...
    class Instance {
        ...
        *
         ...
         */
        fetchNDArrayCache(ndarrayCacheUrl, device, cacheProvider, cacheScope = "tvmjs") {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                let list;
                if (cacheProvider) {
                    list = yield cacheProvider.get(jsonUrl)
                }
                if (!list || Object.keys(list).length === 0) {
                    try {
                        let result = yield fetchRetry(jsonUrl)
                        // console.error('result', result, jsonUrl)
                        // this.env.logger('result', result, jsonUrl)
                        if (result instanceof Response) {
                            list = yield result.json()
                            // console.error('result json')
                            // this.env.logger('result json')
                        }
        
                        if (cacheProvider) {
                            // console.error('put in cache provider', jsonUrl)
                            // this.env.logger('put in cache provider', jsonUrl)
                            cacheProvider.put(jsonUrl, list)
                        }
                    } catch (err) {
                ...
                yield this.fetchNDArrayCacheInternal(ndarrayCacheUrl, list["records"], device, artifactCache, cacheProvider);
         ...
         * @param cacheProvider
         */
        fetchNDArrayCacheInternal(ndarrayCacheUrl, list, device, artifactCache, cacheProvider) {
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    const perf = getPerformance();
                    const tstart = perf.now();
                    ...
                    for (let i = 0; i < list.length; ++i) {
                        ...
                        try {
                            if (cacheProvider) {
                                buffer = yield cacheProvider.get(dataUrl)
                            }

                            if (!buffer) {
                                // const response = yield artifactCache.fetchWithCache(dataUrl);
                                let j = 0;
                                const request = new Request(dataUrl);
                                let response;
                                while (!buffer && j < 8) {
                                    // if (j > 0) console.error('try fetch', j, fetchInterval, dataUrl)
                                    let time1 = performance.now();
                                    try {
                                        response = yield Promise.race([fetch(request), new Promise((resolve) =>
                                            setTimeout(() => {
                                                // console.error('request timeout resolved', dataUrl)
                                                // console.error('i fetch', i, dataUrl)
                                                if (i > 0) resolve()
                                            }, fetchInterval)
                                        )]);
                                        if (response?.statusCode > 399) {
                                            // console.error("Failed to fetch with status code", response.statusCode, dataUrl);
                                            j++;
                                            continue;
                                        }

                                        // let time2 = performance.now();
                                        // fetchInterval = Math.max(Math.min(22000, 6 * (time2 - time1)), 2 * (time2 - time1))
                                        // console.error('reached buffer', response, j, i, dataUrl)

                                        buffer = yield Promise.race([response.arrayBuffer(), new Promise((resolve) =>
                                            setTimeout(() => {
                                                // console.error('arraybuffer timeout resolved', dataUrl)
                                                if (i > 0) resolve()
                                            }, fetchInterval)
                                        )]);
                                        ...
                                        if (!buffer) {
                                            fetchInterval += 10000
                                            // console.error('arraybuffer timeout', j, i, dataUrl)
                                        }

                                        let time2 = performance.now();
                                        fetchInterval = Math.max(Math.min(fetchInterval, 6 * (time2 - time1)), 2 * (time2 - time1))

                                        // console.error('fetch interval', j, fetchInterval)

                                        j++;
                                    } catch (err) {
                                        // console.error("Cannot fetch " + dataUrl, err);
                                        j++;
                                    }
                                }

                                // console.error('buffer', buffer?.byteLength)
                                if (!response) {
                                    throw Error("Cannot fetch " + dataUrl);
                                }
                                if (!buffer) {
                                    throw Error("Cannot read array buffer " + dataUrl);
                                }
                                // const request = new Request(dataUrl);
                                ...
                                //     throw Error("Cannot fetch " + dataUrl);
                                // }
                                // console.error('received response', dataUrl, response.ok, response.status);
                                // const text = yield response.text();
                                // console.error('text', text.substring(0, 20));
                                // request = new Request(dataUrl);
                                // response = yield fetch(request);
                                // if (response === undefined) {
                                //     throw Error("Cannot fetch " + dataUrl);
                                // }
                                // console.error('received second response', dataUrl, response.ok, response.status);
                                // buffer = yield Promise.race([response.arrayBuffer(), new Promise((resolve) =>
                                //     setTimeout(resolve, 10000)
                                // )]);
                                //
                                // if (buffer === undefined) {
                                //     console.error('arraybuffer timeout', dataUrl)
                                //
                                // }
                                // console.error('parsed buffer', dataUrl)

                                if (cacheProvider) {
                                    cacheProvider.put(dataUrl, buffer)
                                }
                            }
                        }
                        catch (err) {
                            console.error("Error: Cannot fetch " + dataUrl + " err= " + err);
                            throw err;
                        }
                        ...
                        for (let j = 0; j < shardRecords.length; ++j) {
                            ...
                            // console.error('buffer before slice', buffer, buffer?.byteLength)
     ...
     */
    class RPCServer {
        constructor(url, key, getImports, logger = () => {}, ndarrayCacheUrl = "", ndarrayCacheDevice = "cpu", initProgressCallback = undefined, asyncOnServerLoad = undefined) {
