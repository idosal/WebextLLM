;(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined"
    ...
    : ((global =
      ...
      factory((global.tvmjs = {}), global.perf_hooks, global.ws))
})(this, function (exports, perf_hooks, ws) {
  "use strict"
  ...
  var environment = createCommonjsModule(function (module, exports) {
    ...
    class Environment {
      constructor(importObject = {}, logger = console.log) {
         ...
         */
        this.packedCFuncTable = [undefined]
        /**
...
`
    ...
    class WebGPUContext {
      ...
      dispose() {
        var _a, _b, _c
        ;(_a = this.canvasRenderManager) === null || _a === void 0
          ? void 0
          : _a.dispose()
        this.bufferTableFreeId = []
        while (this.bufferTable.length != 0) {
          ;(_b = this.bufferTable.pop()) === null || _b === void 0
            ? void 0
            : _b.destroy()
        }
        while (this.podArgStagingBuffers.length != 0) {
          ;(_c = this.podArgStagingBuffers.pop()) === null || _c === void 0
            ? void 0
            : _c.destroy()
        }
        this.device.destroy()
      ...
      runtimeStatsText() {
        let info =
          "peak-memory=" +
          Math.ceil(this.peakAllocatedBytes / (1 << 20)) +
          " MB"
        info +=
          ", all-memory=" +
          Math.ceil(this.allAllocatedBytes / (1 << 20)) +
          " MB"
      ...
      getPodArgsBuffer(nbytes) {
        ...
        if (
          this.podArgStagingBuffers.length >= this.maxNumPodArgsStagingBuffers
        ) {
      ...
      createShadeInternal(finfo, code, asyncMode) {
        ...
        for (let i = 0; i < finfo.arg_types.length; ++i) {
          ...
          if (dtype == "handle") {
            layoutEntries.push({
              ...
              buffer: {
                type: paramWriteAccess[bufferArgIndices.length]
                  ? "storage"
                  : "read-only-storage"
              }
            })
            ...
            throw new Error(
              "Cannot handle argument type " + dtype + " in WebGPU shader"
            )
          }
        }
        ...
        const createShaderFunc = (pipeline) => {
          const submitShader = (...args) => {
            ...
            const numBufferOrPodArgs =
              bufferArgIndices.length + podArgIndices.length
            support.assert(
              args.length == numBufferOrPodArgs + dispatchToDim.length
            )
            ...
            // push pod buffer
            const sizeOfI32 = 4
            const podArgBuffer = this.getPodArgsBuffer(
              (podArgIndices.length + 1) * sizeOfI32
            )
            ...
            if (this.debugLogFinish) {
              ...
              this.device.queue.onSubmittedWorkDone().then(() => {
                console.log(
                  "[" + currCounter + "][Debug] finish shader" + finfo.name
                )
              })
            }
            this.shaderSubmitCounter += 1
  ...
  var runtime = createCommonjsModule(function (module, exports) {
    ...
    class FFILibrary {
      constructor(wasmInstance, imports) {
        ...
        this.memory = new memory.Memory(
          this.detectWasmMemory(this.wasmInstance, imports)
        )
        support.assert(
      ...
      dispose() {
        ...
        ;(_a = this.webGPUContext) === null || _a === void 0
          ? void 0
          : _a.dispose()
      }
      sizeofPtr() {
      ...
      getOrAllocCallStack() {
        ...
        return new memory.CachedCallStack(
          this.memory,
    ...
    class RuntimeContext {
      ...
      attachToCurrentScope(obj) {
        ...
        const currScope =
          this.autoDisposeScope[this.autoDisposeScope.length - 1]
        currScope.push(obj)
        return obj
      }
      moveToParentScope(obj) {
        ...
        const parentScope =
          this.autoDisposeScope[this.autoDisposeScope.length - 2]
    ...
    class NDArray {
      constructor(handle, isView, lib, ctx) {
        ...
        // shape
        const cshapePtr = lib.memory.loadPointer(
          this.dltensor + arrayOffsetShape
        )
        this.shape = []
        ...
        // device
        const deviceType = lib.memory.loadI32(
          this.dltensor + arrayOffsetDevType
        )
        const deviceId = lib.memory.loadI32(this.dltensor + arrayOffsetDevId)
        this.device = new DLDevice(deviceType, deviceId, lib)
        // byte_offset
        this.byteOffset = lib.memory.loadI64(
          this.dltensor + arrayOffsetByteOffset
        )
      }
      /**
      ...
      copyFromRawBytes(data) {
        ...
        if (this.device.deviceType == DeviceStrToEnum.webgpu) {
          ;(_a = this.lib.webGPUContext) === null || _a === void 0
            ? void 0
            : _a.copyRawBytesToBuffer(data, this.getDataPtr(), 0, data.length)
          return this
        }
        // CPU copy
        const size = this.shape.reduce((a, b) => {
    ...
    class Module {
      ...
      importModule(mod) {
        this.lib.checkCall(
          this.lib.exports.TVMModImport(this.getHandle(), mod.getHandle())
        )
      }
    }
    ...
    class TVMObject {
      ...
      typeIndex() {
        ...
        const outPtr = stack.ptrFromOffset(outOffset)
        this.lib.checkCall(
          this.lib.exports.TVMObjectGetTypeIndex(this.getHandle(), outPtr)
        )
      ...
      typeKey() {
        ...
        const outPtr = stack.ptrFromOffset(outOffset)
        this.lib.checkCall(
          this.lib.exports.TVMObjectTypeIndex2Key(type_index, outPtr)
        )
    ...
    class Instance {
      ...
      constructor(wasmModule, importObject = {}, wasmInstance, env) {
        ...
        if (wasmInstance instanceof WebAssembly.Instance) {
          support.assert(
            env instanceof environment.Environment,
      ...
      listGlobalFuncNames() {
        ...
        this.lib.checkCall(
          this.exports.TVMFuncListGlobalNames(outSizePtr, outArrayPtr)
        )
      ...
      getGlobalFuncInternal(name, autoAttachToScope = true) {
        ...
        stack.commitToWasmMemory(outOffset)
        this.lib.checkCall(
          this.exports.TVMFuncGetGlobal(stack.ptrFromOffset(nameOffset), outPtr)
        )
      ...
      isPackedFunc(func) {
        ...
        return (
          typeof func == "function" && func.hasOwnProperty("_tvmPackedCell")
        )
      }
      /**
      ...
      getParamsFromCache(prefix, numParams) {
        return this.ctx
          .paramModuleFromCache(prefix, new Scalar(numParams, "int32"))
      ...
      fetchNDArrayCache(ndarrayCacheUrl, device, cache) {
        return __awaiter(this, void 0, void 0, function* () {
          ...
          yield this.fetchNDArrayCacheInternal(
            ndarrayCacheUrl,
            list["records"],
            device,
            cache
          )
          this.cacheMetadata = Object.assign(
            Object.assign({}, this.cacheMetadata),
            list["metadata"]
          )
        })
      }
       ...
       * @param cache
       */
      fetchNDArrayCacheInternal(ndarrayCacheUrl, list, device, cache) {
        return __awaiter(this, void 0, void 0, function* () {
          const perf = compact.getPerformance()
          let tstart = perf.now()
          let totalBytes = 0
          ...
          const reportCallback = (iter) => {
            ...
            for (let j = 0; j < this.initProgressCallback.length; ++j) {
              let text =
                "Fetching param cache[" + iter + "/" + list.length + "]: "
              text +=
                Math.ceil(fetchedBytes / (1024 * 1024)).toString() +
                "MB fetched. "
              text +=
                Math.floor((fetchedBytes * 100) / totalBytes).toString() +
                "% completed, "
          ...
          for (let i = 0; i < list.length; ++i) {
            ...
            try {
              if (cache) {
                buffer = yield cache.get(dataUrl)
              }
              ...
              if (!buffer) {
                ...
                try {
                  if (cache) {
                    yield cache.put(dataUrl, buffer)
                  }
                } catch (err) {
                  this.env.logger(
                    "Error: Cannot cache " +
                      dataUrl +
                      ", reloading will be slow"
                  )
                }
              }
            } catch (err) {
      ...
      empty(shape, dtype = "float32", dev = this.device("cpu", 0)) {
        ...
        const ret = this.ctx.attachToCurrentScope(
          new NDArray(
            this.memory.loadPointer(outPtr),
            false,
            this.lib,
            this.ctx
          )
        ...
        if (dataRGBA.device.deviceType != DeviceStrToEnum.webgpu) {
          throw new Error(
            "Can only run showImage on WebGPU array, " +
              "get " +
              DeviceEnumToStr[dataRGBA.device.deviceType] +
              " instead."
          )
        }
          ...
          for (const [key, finfo] of fmapEntries) {
            ...
            const event = webgpuContext
              ...
              .then(() => {
                ...
                if (
                  tend - tlastReport < 1000 &&
                  finishCounter != fmapEntries.length
                ) {
                  return
                }
                tlastReport = tend
                ...
                for (let j = 0; j < this.initProgressCallback.length; ++j) {
                  ...
                  let text =
                    ...
                    "]: "
                  text +=
                    Math.floor(progress * 100).toString() + "% completed, "
                  text += timeElapsed + " secs elapsed."
                  this.initProgressCallback[j]({
                    progress: progress,
                    timeElapsed: timeElapsed,
      ...
      initWebGPU(device) {
        ...
        this.registerAsyncServerFunc("wasm.WebGPUWaitForTasks", () =>
      ...
      createPackedFuncFromCFunc(func) {
        ...
        const outPtr = stack.ptrFromOffset(outOffset)
        this.lib.checkCall(
          this.exports.TVMWasmFuncCreateFromCFunc(findex, outPtr)
        )
      ...
      wrapJSFuncAsPackedCFunc(func) {
        ...
        return (
          ...
          if (rv !== undefined && rv !== null) {
            ...
            stack.commitToWasmMemory()
            lib.checkCall(
              lib.exports.TVMCFuncSetReturn(ret, valuePtr, codePtr, 1)
            )
      ...
      retValueToJS(rvaluePtr, tcode, callbackArg) {
        switch (tcode) {
          ...
          case 11 /* TVMStr */: {
            const ret = this.memory.loadCString(
              this.memory.loadPointer(rvaluePtr)
            )
            return ret
          }
  ...
  var rpc_server = createCommonjsModule(function (module, exports) {
    ...
    ;(function (RPCServerState) {
      ...
      RPCServerState[(RPCServerState["WaitForCallback"] = 3)] =
        "WaitForCallback"
      RPCServerState[(RPCServerState["ReceivePacketHeader"] = 4)] =
        "ReceivePacketHeader"
      RPCServerState[(RPCServerState["ReceivePacketBody"] = 5)] =
        "ReceivePacketBody"
    ...
    class RPCServer {
      ...
      onClose(_event) {
        ...
        if (this.state == RPCServerState.ReceivePacketHeader) {
          ...
          new RPCServer(
            ...
            this.getImports,
            this.logger,
            this.ndarrayCacheUrl,
            this.ndarrayCacheDevice,
            this.initProgressCallback,
            this.asyncOnServerLoad
          )
        } else {
      ...
      processEvents() {
        while (
          this.buffredBytes >= this.pendingBytes &&
          this.pendingBytes != 0
        ) {
          this.onDataReady()
        }
      ...
      onInitServer(args, header, body) {
        ...
        support.assert(this.pendingBytes == 0)
        const asyncInitServer = () =>
          __awaiter(this, void 0, void 0, function* () {
            support.assert(args[1] instanceof Uint8Array)
            const inst = yield runtime.instantiate(
              args[1].buffer,
              this.getImports(),
              this.logger
            )
            ...
            if (this.ndarrayCacheUrl.length != 0) {
              if (this.ndarrayCacheDevice == "cpu") {
                yield this.inst.fetchNDArrayCache(
                  this.ndarrayCacheUrl,
                  this.inst.cpu()
                )
              } else {
            ...
            const messageHandler = fcreate(
              (cbytes) => {
                ...
                if (this.socket.readyState == 1) {
                  ...
                  const sendDataWithCongestionControl = () =>
                    __awaiter(this, void 0, void 0, function* () {
      ...
      handleInitHeader() {
        const reader = new ByteStreamReader(
          this.readFromBuffer(4 /* I32 */ * 2)
        )
        const magic = reader.readU32()
        if (magic == RPC_MAGIC + 1) {
          throw new Error(
            "key: " + this.key + " has already been used in proxy"
          )
        } else if (magic == RPC_MAGIC + 2) {
          throw new Error(
            "RPCProxy do not have matching client key " + this.key
          )
        }
  ...
  var dist = createCommonjsModule(function (module, exports) {
    ...
    Object.defineProperty(exports, "DLDevice", {
      enumerable: true,
      get: function () {
        return runtime.DLDevice
      }
    })
    Object.defineProperty(exports, "DLDataType", {
      enumerable: true,
      get: function () {
        return runtime.DLDataType
      }
    })
    ...
    Object.defineProperty(exports, "TVMArray", {
      enumerable: true,
      get: function () {
        return runtime.TVMArray
      }
    })
    ...
    Object.defineProperty(exports, "wasmPath", {
      enumerable: true,
      get: function () {
        return support.wasmPath
      }
    })

    Object.defineProperty(exports, "detectGPUDevice", {
      enumerable: true,
      get: function () {
        return webgpu.detectGPUDevice
      }
    })
